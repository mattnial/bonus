<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="text-xs text-right text-gray-500">
            Usuario: <span class="font-bold text-blue-800" id="docUserDisplay">Admin</span>
            <input type="hidden" id="currentStaffId" value="">
            <input type="hidden" id="currentStaffName" value="">
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
        ...
        <form id="docForm" class="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div id="f-motivo" class="dyn-field hidden space-y-3">
                ...
                <input type="date" name="date_start" id="dateStartPicker" class="w-full border rounded p-2 text-sm">
            </div>
...
<script>
// --- INICIALIZACIÓN PROPIA DEL MÓDULO ---
// Como estamos en v3, necesitamos obtener el usuario desde localStorage porque PHP SESSION está desactivada
(function() {
    // 1. Cargar fecha actual
    document.getElementById('dateStartPicker').value = new Date().toISOString().split('T')[0];

    // 2. Cargar datos del staff desde localStorage
    const staff = JSON.parse(localStorage.getItem('vilcanet_staff') || '{}');
    if(staff.id) {
        document.getElementById('currentStaffId').value = staff.id;
        document.getElementById('currentStaffName').value = staff.name;
        document.getElementById('docUserDisplay').textContent = staff.name;
    }
})();
