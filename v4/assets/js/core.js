...
async function renderDynamicMenu() {
    ...
    try {
        ...
        window.APP_MENU_CONFIG.forEach(item => {
            ...
            const hasAccess = (item.id === 'home' || allowedIds.includes(item.id));
            ...
            if (hasAccess) {
                ...
                if (item.type === 'dropdown') {
                        ...
                        <div class="mb-1">
                            ...
                            <div id="${dropId}" class="hidden pl-10 pr-2 mt-1 space-y-1 overflow-hidden transition-all duration-300">
                                ${item.subitems.map(sub => `
                                    <button onclick="${sub.action}" class="w-full text-left py-2 px-3 text-xs font-medium text-slate-500 hover:text-blue-400 hover:bg-slate-800/30 rounded-md transition-colors border-l-2 border-transparent hover:border-blue-500">
                                        ${sub.label}
                                    </button>
                                `).join('')}
                            </div>
                    ...
                    container.insertAdjacentHTML('beforeend', `
                        <div class="mb-1">
                            <button onclick="switchView('${item.view}')" id="menu-btn-${item.id}"
